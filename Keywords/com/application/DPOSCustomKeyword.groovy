...
public class DPOSCustomKeyword {

	@Keyword
	def needSomeHelpCloseButton(){

		if(WebUI.waitForElementVisible(findTestObject('Object Repository/Common/button_close_Need Some Help'), 1)){
			WebUI.click( findTestObject('Object Repository/Common/button_close_Need Some Help') )
		}
		else{
	...
	def randomString(int length) {
		...
		String chars = "This is automation Testing Script Execution"
	...
	@Keyword
	def privacyAndConsentCloseButton(){

		if(WebUI.waitForElementVisible(findTestObject('Object Repository/Common/button_close_ForPrivacyAndConsents'),1 ) ){
			WebUI.click( findTestObject('Object Repository/Common/button_close_ForPrivacyAndConsents') )
		}
		else{
	...
	def gettestdatafix() {
		try{
			...
			FileInputStream file = new FileInputStream (new File(GlobalVariable.G_AppPath+"/Data Files/Test_Data.xlsx"))
			XSSFWorkbook workbook = new XSSFWorkbook(file);
			XSSFSheet sheet1 = workbook.getSheetAt(0);

			GlobalVariable.AppURL = sheet1.getRow(1).getCell(0).getStringCellValue();
			println "Application URL is: "+GlobalVariable.AppURL
			file.close();
		}catch(Exception e){
